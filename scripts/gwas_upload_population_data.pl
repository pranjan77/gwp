...
my $population_object        = $ARGV[4]; #population object name to save
my $environment              = $ARGV[5]; #web for web uploader, "local" for command line uploader  
...
if ($environment eq "local"){
  ...
  my ($kbase_genome_id, $GwasPopulation_description, $originator, $pubmed_id, $comment) = @datax;
  chomp($comment);
...
sub print_usage {
  &return_error("USAGE: gwas_upload_population.pl ws_url ws_id metadata data obj_name environment");
}
